...
### 🔍 智能特征检测
- **7个关键特征识别**：标准logo、标准类别、标准号、第一横线、标准名称、发布时间、发布单位
...
### 检测标准（已调整）
- **特征数量要求**：7个特征中必须检测到≥4个（从5降低到4）
- **关键特征要求**：特征4（第一横线）、特征5（标准名称）、特征6（发布时间）中至少满足2个
- **位置符合度**：基于confidence值计算，要求>0.6（从0.7降低到0.6）
- **模板相似度**：与标准模板比对，相似度要求>0.2（从0.3降低到0.2）
...
**注意**：系统仅处理PDF文件的第一页进行特征检测，这通常足以识别标准文档的封面页。
...
[1/150] document1.pdf
  第一页: 6/7 特征
  ✅ 第一页满足条件（特征数6>=5，关键特征齐全，位置符合度0.85，模板相似度0.45）
