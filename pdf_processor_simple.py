...
import io
from main_simple import SimpleStandardDocumentFeatureExtractor
...
class SimplePDFProcessor:
    ...
    def analyze_page_simple(self, image, page_num):
        """使用简化版特征提取器分析页面"""
        temp_path = f"temp_page_{page_num}.png"
        cv2.imwrite(temp_path, image)
        try:
            extractor = SimpleStandardDocumentFeatureExtractor(temp_path)
    ...
    def process_pdf(self, pdf_path):
        ...
        result = self.analyze_page_simple(image, 1)
        ...
        if result and 'error' not in result:
            ...
            for region_name, analysis in region_analysis.items():
                ...
                print(f"    {region_name_cn}: 留白{analysis['whitespace_ratio']:.3f}, "
                      ...
                      f"文本密度{analysis['text_features']['text_density']:.3f}")
    ...
    def process_all(self):
        """批量处理所有PDF文件"""
        print("PDF 文件批量处理 - 简化版（基于mb5.png，无OCR依赖）")
        print("=" * 60)
        ...
        print("- 统计各区域留白比例")
        print("- 使用图像特征分析代替OCR文字识别")
        print("- 结合mb4.png的文字分布特征")
        print("- 无需安装Tesseract OCR")

def main():
    """主函数"""
    print("PDF标准文档分类系统 - 简化版")
    print("基于mb5.png的3个蓝色框区域分析（无OCR依赖）")
    print("=" * 60)
    
    # 创建处理器实例
    processor = SimplePDFProcessor(source_drive="E:", target_folder="jc")
